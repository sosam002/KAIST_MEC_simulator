...
import environment3_ppo_tobe_uploaded as environment
import pickle
from rl.ppo.ppo3 import PPO
from rl.ppo.ppo_utils import *

def main():

    ############## save parameters ##############
    file_name = 'ppo3_'+str(datetime.now())
    ...
    for i_episode in range(1, max_episodes+1):
        state = env.reset()
        max_timesteps = min((i_episode//100+1)*400, 4000)
        ...
        evaluations_fixed_len.append(evaluate_policy(env, ppo, cloud_policy, memory))
        np.save("{}/eval".format(result_dir), evaluations)
        np.save("{}/eval_100steps".format(result_dir), evaluations_fixed_len)
