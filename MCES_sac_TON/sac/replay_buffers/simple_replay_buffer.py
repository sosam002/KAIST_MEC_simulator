...
from .replay_buffer import ReplayBuffer, ReplayBufferDAC
...
class SimpleReplayBufferDAC(ReplayBufferDAC, Serializable):
    def __init__(self, env_spec, max_replay_buffer_size):
        super(SimpleReplayBufferDAC, self).__init__()
        Serializable.quick_init(self, locals())
    ...
    def add_sample(self, observation, action, reward, terminal, raw_action,
                   next_observation, **kwargs):
        self._observations[self._top] = observation
        self._actions[self._top] = action
        self._rewards[self._top] = reward
        self._terminals[self._top] = terminal
        self._raw_actions[self._top] = raw_action
        self._next_obs[self._top] = next_observation
    ...
    def random_batch(self, batch_size):
        ...
        return dict(
            ...
            rewards=self._rewards[indices],
            terminals=self._terminals[indices],
            raw_actions=self._raw_actions[indices],
            next_observations=self._next_obs[indices],
        )
    ...
    def __getstate__(self):
        d = super(SimpleReplayBufferDAC, self).__getstate__()
        d.update(dict(
            o=self._observations.tobytes(),
            a=self._actions.tobytes(),
            r=self._rewards.tobytes(),
    ...
    def __setstate__(self, d):
        super(SimpleReplayBufferDAC, self).__setstate__(d)
        self._observations = np.fromstring(d['o']).reshape(
            self._max_buffer_size, -1
        )
        self._next_obs = np.fromstring(d['no']).reshape(
        ...
        self._rewards = np.fromstring(d['r']).reshape(self._max_buffer_size)
        self._terminals = np.fromstring(d['t'], dtype=np.uint8)
        self._raw_actions = np.fromstring(d['ra']).reshape(self._max_buffer_size, -1)
        self._top = d['top']
        self._size = d['size']
