...
class SimpleSamplerDAC(Sampler):
    def __init__(self, **kwargs):
        super(SimpleSamplerDAC, self).__init__(**kwargs)
    ...
    def sample(self):
        ...
        action, raw_action = self.policy.get_action_with_raw(self._current_observation)
        next_observation, reward, terminal, info = self.env.step(action)
        self._path_length += 1
        self._path_return += reward
        self._total_samples += 1
    ...
    def log_diagnostics(self):
        super(SimpleSamplerDAC, self).log_diagnostics()
