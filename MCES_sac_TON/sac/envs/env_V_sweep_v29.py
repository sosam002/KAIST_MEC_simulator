...
from sac.envs.servernode_w_appqueue_w_appinfo_cores import ServerNode as Edge
from sac.envs.servernode_w_totalqueue_cores import ServerNode as Cloud
from sac.envs.environment import Environment
from sac.envs.applications import *
from sac.envs.channels import *
from sac.envs.constants import *
from sac.envs.rl_networks.utils import *
import gym
...
class MEC_v29(gym.Env):
    def __init__(self, task_rate=10, applications=(SPEECH_RECOGNITION, NLP, FACE_RECOGNITION), time_delta=1, use_beta=True, empty_reward=True, cost_type=COST_TYPE, max_episode_steps=5000, time_stamp=0):
        ...
        self.lambdas = np.array( [ arrival_bits(SPEECH_RECOGNITION, dist='deterministic')*get_info(SPEECH_RECOGNITION, "popularity"), \
            arrival_bits(NLP, dist='deterministic')*get_info(NLP, "popularity"), \
            arrival_bits(FACE_RECOGNITION, dist='deterministic')*get_info(FACE_RECOGNITION, "popularity")] )*self.task_rate/GHZ
    ...
    def step(self, action, use_beta=True, generate=True):
        ...
        q1=np.array(self.get_edge_qlength(scale=GHZ)) #q(t)+a(t)
        a_t = start_state[3:6]
        ...
        cost = self.get_cost(used_edge_cpus, used_cloud_cpus, q0, bt, cost_type=self.cost_type)
    ...
    def get_cost(self, used_edge_cpus, used_cloud_cpus,before, bt, cost_type, failed_to_offload=0, failed_to_generate=0):
