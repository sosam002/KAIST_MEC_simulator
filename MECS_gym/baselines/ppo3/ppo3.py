...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        with tf.variable_scope('ppo3_model', reuse=tf.AUTO_REUSE):
        ...
        params = tf.trainable_variables('ppo3_model')
        # 2. Build our trainer
        if MPI is not None:
...
class Runner(AbstractEnvRunner):
    ...
    def __init__(self, *, env, model, nsteps, gamma, lam):
        ...
        self.ret_buffer = np.zeros(self.buff_size)
        self.total_len = 0
    ...
    def rewfilt(self, rews):
        rews = np.clip(rews / np.sqrt(np.var(self.ret_buffer[:np.minimum(self.total_len,self.buff_size)]) + self.epsilon), -self.cliprew, self.cliprew)
        return rews
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            if self.total_len<len(self.ret_buffer):
                self.ret_buffer[self.total_len] = self.ret
            else:
                self.ret_buffer[self.total_len % self.buff_size]=self.ret
        ...
        mb_rewards = self.rewfilt(np.asarray(mb_rewards, dtype=np.float32))
        mb_actions = np.asarray(mb_actions)
        mb_values = np.asarray(mb_values, dtype=np.float32)
        mb_neglogpacs = np.asarray(mb_neglogpacs, dtype=np.float32)
        mb_dones = np.asarray(mb_dones, dtype=np.bool)
        last_values = self.model.value(self.obfilt(self.obs), S=self.states, M=self.dones)
...
class EvalRunner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            # Given observations, get action value and neglopacs
            # We already have self.obs because Runner superclass run self.obs[:] = env.reset() on init
            actions, logstd = self.model.meanlogstd(self.obfilt(self.obs))
            mb_obs.append(self.obs.copy())
            mb_actions.append(actions)
            mb_dones.append(self.dones)
        ...
        return mb_obs.swapaxes(0, 1).reshape(s[0]*s[1],*s[2:]), epinfos
...
def learn(*, network, env, total_timesteps, sess=None, fixstd=False, logstd_init=0, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    print("obdim : ", ob_space.shape[0])
    print("acdim : ", ac_space.shape[0])
    ...
    for update in range(1, nupdates+1):
        ...
        edge_s = np.transpose(obs)[:15].reshape(-1, 3, len(obs))
        cloud_s = np.vstack([path['cloud_cpu_used'] for path in epinfos])

        edge_queue = edge_s[2]-edge_s[1]  # shape (episode length, 8)
        edge_cpu = edge_s[3]
        workload = edge_s[4]
        cloud_cpu = cloud_s[0]
        ...
        if eval_env is not None:
            eval_obs, eval_epinfos = eval_runner.run() #pylint: disable=E0632
            eval_edge_s = np.transpose(eval_obs)[:15].reshape(-1, 3, len(eval_obs))
            eval_cloud_s = np.vstack([path['cloud_cpu_used'] for path in eval_epinfos])
            eval_edge_queue = eval_edge_s[2] - eval_edge_s[1]  # shape (episode length, 8)
            eval_edge_cpu = eval_edge_s[3]
            eval_workload = eval_edge_s[4]
            # eval_cloud_queue = eval_cloud_s[2]
            eval_cloud_cpu = eval_cloud_s[0]
        ...
        if update % log_interval == 0 or update == 1:
            ...
            for i in range(200):
                start_ = i * 100
                end_ = (i+1) * 100
                logger.logkv("eval_q_1_%02d%02d"%(i,i+1), np.mean(eval_edge_queue[0, start_:end_]))
                logger.logkv("eval_q_2_%02d%02d"%(i,i+1), np.mean(eval_edge_queue[1, start_:end_]))
                logger.logkv("eval_q_3_%02d%02d"%(i,i+1), np.mean(eval_edge_queue[2, start_:end_]))
                logger.logkv("eval_q_var_%02d%02d"%(i,i+1), np.mean(np.var(eval_edge_queue[:3, start_:end_], axis=0)))
            logger.logkv("eval_q_1_all", np.mean(eval_edge_queue[0,:]))
            logger.logkv("eval_q_2_all", np.mean(eval_edge_queue[1,:]))
            logger.logkv("eval_q_3_all", np.mean(eval_edge_queue[2,:]))
            logger.logkv("eval_q_var_all", np.mean(np.var(eval_edge_queue[:3,:],axis=0)))
