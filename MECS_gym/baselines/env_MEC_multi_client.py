...
class MEC_v0(gym.Env):
    ...
    def init_linked_pair(self, c_cores, c_clk, s_cores, s_clk, up_channel, down_channel=None, c_random_task_generating=True):
        ...
        state = self._get_obs(client, scale=GHZ)
    ...
    def init_linked_client(self, c_cores, c_clk, server_id, up_channel, down_channel=None, c_random_task_generating=True):
        client = self.add_client(c_cores, c_clk, c_random_task_generating)
        self.add_link(client, self.servers[server_id], up_channel, down_channel)
    ...
    def reset(self, rand_start = 0):
        ...
        for client in self.clients.values():
            _, failed_to_generate, _ = self._step_generation(client)
            reset_states.append( self._get_obs(client, scale=GHZ) )
        # reset_state = reset_state[:-3]
        # reset_state[-1] = 0.0
        return reset_states

    def _get_obs(self, client, scale=GHZ):
        return np.array( client._get_obs(self.timestamp, scale=scale) )
    ...
    def step(self, action, use_offload=True, generate=True):
        ...
        new_states, costs, dones, infos = [],[],[],[]
        n_agents = len(c_action)
        for client, c_action in list(zip(self.clients.values(), action)):
            self._step_move(client)
            q1=np.array(self.get_edge_qlength(client, scale=GHZ))
            action_alpha, action_beta = list(), list()
            if self.use_offload:
                action_alpha = c_action.flatten()[:int(self.action_dim/2)].reshape(1,-1)
                action_beta = c_action.flatten()[int(self.action_dim/2):].reshape(1,-1)
                ### softmax here
                action_beta = softmax_1d(action_beta)
            else:
                action_alpha = c_action
            ### softmax here
            action_alpha = softmax_1d(action_alpha)
            ##########################################################3

            used_edge_cpus = self._step_alpha(client, action_alpha)
            used_cloud_cpus = self._step_beta(client, action_beta)
            ...
            q_last=np.array(self.get_edge_qlength(client, scale=GHZ)) #############

            _, failed_to_generate, _ = self._step_generation(client) #########

            # 원래 스테이트에서는 비율로 들어가서 /216 했었음. 지금은 216 곱해진 값
            new_states.append( self._get_obs(client, scale=GHZ) ) ###############
            costs.append(self.get_cost(used_edge_cpus, temp_req_cloud_cpus, q1, q_last, cost_type=self.cost_type))
        ...
        if self.timestamp == self.max_episode_steps:
            return new_state, -costs, [1]*self.n_agents, [{"cloud_cpu_used":cloud_info}]*self.n_agents
        return new_states, -cosst, [0]*self.n_agents, [{"cloud_cpu_used":cloud_info}]*self.n_agents

    def _step_move(self, node):
        if node.movable:
            node.move()

    def _step_alpha(self, node, action):
        # initial_qlength= self.get_total_qlength()
        used_edge_cpus = collections.defaultdict(float)
        action = action.flatten()[:-1].reshape(1,-1)
        used_edge_cpus[node.get_uuid()] = node.do_tasks(action)
        return used_edge_cpus


    def _step_beta(self, node, action):
        tasks_to_be_offloaded = collections.defaultdict(dict)
        used_cloud_cpus = collections.defaultdict(float)
        action = action.flatten()[:-1].reshape(1,-1)
        higher_nodes = node.get_higher_node_ids()

        for higher_node in higher_nodes:
            task_to_be_offloaded, failed = node.offload_tasks(beta, higher_node)
    ...
    def _step_generation(self, node):
        initial_qlength= self.get_edge_qlength(node)
        if not self.silence: print("###### random task generation start! ######")
        arrival_size, failed_to_generate = node.random_task_generation(self.task_rate, self.timestamp, *self.applications)
        if not self.silence: print("###### random task generation ends! ######")

        after_qlength = self.get_edge_qlength(node, scale=GHZ)

        return initial_qlength, failed_to_generate, after_qlength

    def get_edge_qlength(self, node, scale=1):
        qlengths = list()
