...
class DiagGaussianSquashPdType(PdType):
    def __init__(self, size):
        self.size = size
    def pdclass(self):
        return DiagGaussianSquashPd
...
class DiagGaussianVarStdPdType(PdType):
    def __init__(self, size):
        self.size = size
    def pdclass(self):
        return DiagGaussianVarStdPd

    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        latent = fc(latent_vector, 'pi', 2*self.size, init_scale=init_scale, init_bias=init_bias)
        mean, logstd = tf.split(axis=len(latent.shape) - 1, num_or_size_splits=2, value=latent)
        return self.pdfromflat(latent), mean
...
class DiagGaussianSquashPd(Pd):
    ...
    def neglogp(self, action):
        ...
        return 0.5 * tf.reduce_sum(tf.square((x - self.mean) / self.std), axis=-1) \
               + 0.5 * np.log(2.0 * np.pi) * tf.to_float(tf.shape(x)[-1]) \
               + tf.reduce_sum(self.logstd, axis=-1) + tf.reduce_sum(tf.log(1 - tf.tanh(x) ** 2 + 10**(-8)), axis=-1)
    def kl(self, other):
        assert isinstance(other, DiagGaussianPd)
        return tf.reduce_sum(other.logstd - self.logstd + (tf.square(self.std) + tf.square(self.mean - other.mean)) / (2.0 * tf.square(other.std)) - 0.5, axis=-1)
    def entropy(self):
...
def make_pdtype(ac_space, VARSTD = False, SQAUSH = False):
    from gym import spaces
    if isinstance(ac_space, spaces.Box):
        assert len(ac_space.shape) == 1
        if VARSTD:
            return DiagGaussianFixStdPdType(ac_space.shape[0])
        elif SQAUSH:
            return DiagGaussianSquashPdType(ac_space.shape[0])
        else:
