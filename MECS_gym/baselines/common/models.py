...
@register("forMEC")
def forMEC(num_layers=2, num_hidden=64, activation=tf.tanh, layer_norm=False):
    ...
    def network_fn(X):
        ...
        with tf.variable_scope("forMEC_fc_edge") as scope:
            w = tf.get_variable("w", [nin_edge, nh], initializer=ortho_init(np.sqrt(2)))
        ...
        h_e = fc(h_e, 'forMEC_fc_edge2', nh=int(num_hidden)/2, init_scale = np.sqrt(2))
        h_e = activation(h_e)
