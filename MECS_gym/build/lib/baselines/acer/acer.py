            ...
            if self.eval_env is not None:
                ...
                for t_rollout in range(10000000):
                    ...
                    eval_obs, eval_r, eval_done, eval_info = self.eval_env.step(
                        eval_action)  # scale for execution in env (as far as DDPG is concerned, every action is in [-1, 1])
                    ...
                    if epilen >= 10:
                        break
            if self.eval_env is not None:
                logger.record_tabular('eval_eplenmean', np.mean(self.safemean([epinfo['l'] for epinfo in epinfos])))
                logger.record_tabular('eval_eprewmean', np.mean(self.safemean([epinfo['r'] for epinfo in epinfos])))
...
def learn(network, env, eval_env=None, seed=None, nsteps=20, total_timesteps=int(80e6), q_coef=0.5, ent_coef=0.01,
    ...
    **network_kwargs:               keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if replay_ratio > 0:
        ...
        buffer = None
    nbatch = nenvs*nsteps
    acer = Acer(runner, model, buffer, log_interval, eval_env)
