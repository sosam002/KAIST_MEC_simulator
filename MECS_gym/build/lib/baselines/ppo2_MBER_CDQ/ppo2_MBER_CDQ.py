...
from baselines.common.policies_double import build_policy
...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        with tf.variable_scope('ppo2_target_model', reuse=tf.AUTO_REUSE):
            act_model_target = policy(nbatch_act, 1, sess)

            # Train model for training
            train_model_target = policy(None, nsteps, sess)
        ...
        # Unclipped value
        vf_losses1 = tf.maximum(tf.square(vpred - R),tf.square(vpredclipped - R))
        # Clipped value
        vf_losses2 = tf.maximum(tf.square(vpred2 - R),tf.square(vpredclipped2 - R))

        vf_loss = .5 * tf.reduce_mean(vf_losses1) +.5 * tf.reduce_mean(vf_losses2)

        # Calculate ratio (pi current policy / pi old policy)
        ratio = tf.exp(OLDNEGLOGPAC - neglogpac) * RHO_NOW #* tf.minimum(1.0,RHO_NOW)
        ...
        params_vf_target = tf.trainable_variables('ppo2_target_model/vf')
        params_vf = tf.trainable_variables('ppo2_model/vf')
        target_ops = [tf.assign(target, (1 - 0.005) * target + 0.005 * source) for target, source in zip(params_vf_target, params_vf)]
        def train(lr, cliprange, klconst, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, rho_now, kl_rest, states=None):
            # Here we calculate advantage A(s,a) = R + yV(s') - V(s)
            # Returns = R + yV(s')
            # Normalize the advantages
            advs = (advs - advs.mean()) / (advs.std() + 1e-8)
            td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                    CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values, MEANNOW:mean_now, LOGSTDNOW:logstd_now, KLCONST:klconst, RHO_NOW:rho_now, KL_REST:kl_rest}
            if states is not None:
                td_map[train_model.S] = states
                td_map[train_model.M] = masks
            return sess.run(
                [pg_loss, vf_loss, entropy, approxkl, clipfrac, kloldnew, approxoldkl, target_ops, _train],
                td_map
            )[:-2]
        ...
        self.value2 = act_model.value2
        self.values2 = train_model.value2

        self.value_target = act_model_target.value
        self.values_target = train_model_target.value

        self.value_target2 = act_model_target.value2
        self.values_target2 = train_model_target.value2
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, adaptive_clip=0, trunc_rho=1.0, useadv = 0, vtrace = 0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    def add_vtarg_and_adv(seg, gamma, value, value_target, lam):
        """
    ...
    def add_vtarg_and_adv_vtrace(seg, gamma, value, value_target, values2, values2_target, rho, trunc_rho, acdim=None):
        """
        Compute target value using TD(lambda) estimator, and advantage with GAE(lambda)
        """
        done = np.append(seg["done"], 0)  # last element is only used for last vtarg, but we already zeroed it if last new = 1
        ...
        rew = seg["rew"]
        lastgaelam = 0
        min_value_target = np.minimum(value_target,values2_target)
        for t in reversed(range(T)):
            nonterminal = 1 - done[t + 1]
            delta = r[t] * (rew[t] + gamma * min_value_target[t + 1] * nonterminal - min_value_target[t])
            gaelam[t] = lastgaelam = delta + gamma * c[t + 1] * nonterminal * lastgaelam
        ret = gaelam + min_value_target[:-1]
    ...
    params_vf_target = tf.trainable_variables('ppo2_target_model/vf')
    print(params_vf)
    print(params_vf_target)
    model.sess.run([tf.assign(target, source) for target, source in zip(params_vf_target, params_vf)])
    ...
    for update in range(1, nupdates+1):
        ...
        values_target = model.values_target(ob_stack)
        values[:-1] = (1.0-final_done) * values[:-1]
        values_target[:-1] = (1.0-final_done) * values_target[:-1]

        values2 = model.values2(ob_stack)
        values_target2 = model.values_target2(ob_stack)
        values2[:-1] = (1.0-final_done) * values2[:-1]
        values_target2[:-1] = (1.0-final_done) * values_target2[:-1]
        print(values)
        print(values_target)
        print(values2)
        print(values_target2)
        ...
        # print(rho)
        if vtrace:
            adv, ret, gae = add_vtarg_and_adv_vtrace(seg, gamma, values, values_target, values2, values_target2, rho, trunc_rho)
            print(adv)
        ...
        for _ in range(noptepochs):
            ...
            for _ in range(int(2048/64)):
                ...
                loss_epoch = model.train(lrnow, cliprangenow, klconst, *slices)
            ...
            # # print(rho)
            # if vtrace:
            #     adv, ret, gae = add_vtarg_and_adv_vtrace(seg, gamma, values, values_target, values2, values_target2,
            #                                              rho, trunc_rho)
