...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, klconst, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, rho_now, states=None):
            # Here we calculate advantage A(s,a) = R + yV(s') - V(s)
            # Returns = R + yV(s')
            # Normalize the advantages
            advs = (advs - advs.mean()) / (advs.std() + 1e-8)
            td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                    CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values, MEANNOW:mean_now, LOGSTDNOW:logstd_now, KLCONST:klconst, RHO_NOW:rho_now}
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, adaptive_clip=0, trunc_rho=1.0, vtrace = 0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    for update in range(1, nupdates+1):
        ...
        for _ in range(noptepochs):
            ...
            for _ in range(int(2048/64)):
                ...
                slices = (arr[idx] for arr in (seg["ob"], ret, gae, seg["done"], seg["ac"], values[:-1], neglogpnow, seg["mean"], seg["logstd"], rho))
            ...
            rho = np.exp(-neglogpn + neglogpnow)
            prior_row = np.zeros(len(seg["ob"]))
            temp=[]
            for i in range(int(len(prior_row) / 2048)):
                temp_row = np.mean(np.abs(rho[i * 2048:(i + 1) * 2048] - 1.0) + 1.0)
