...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, klconst, rgae, trunc_rho, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, kl_rest, rho_now, states=None):
            ...
            notuse_num = np.sum(((advs > 0) & (rho_now>1+0.2)) + ((advs < 0) & (rho_now<1-0.2)))
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        if save_interval and (update % save_interval == 0 or update == 1) and logger.get_dir() and (MPI is None or MPI.COMM_WORLD.Get_rank() == 0):
            ...
            model.save(savepath)
    np.savetxt('/home/han/Downloads/log_ppo/graphdata/batchIS_Humanoid.txt',np.array(batch_IS))
    np.savetxt('/home/han/Downloads/log_ppo/graphdata/notuse_Humanoid.txt', np.array(not_use))
    return model
