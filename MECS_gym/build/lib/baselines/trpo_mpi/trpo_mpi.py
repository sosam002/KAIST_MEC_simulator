    ...
    while True:
        ...
        if eval_env is not None:
            ...
            for t_rollout in range(10000000):
                eval_action, eval_q, _, _ = pi.step(eval_obs, stochastic=False)
                ...
                if epilen >= 10:
                    break
        if eval_env is not None:
            logger.record_tabular('eval_eplenmean', np.mean(safemean([epinfo['l'] for epinfo in epinfos])))
            logger.record_tabular('eval_eprewmean', np.mean(safemean([epinfo['r'] for epinfo in epinfos])))
