...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        ones = tf.ones_like(logstd)
        ratio_mean = tf.exp(tf.reduce_sum(- 0.5 * tf.square((A - mean) / ones) + 0.5 * tf.square((A - MEANNOW) / ones),axis=-1))
        ratio_std = tf.exp(tf.reduce_sum(- 0.5 * tf.square(tf.stop_gradient(A - mean) / tf.exp(logstd)) - logstd + 0.5 * tf.square(
            tf.stop_gradient(A - MEANNOW) / tf.exp(LOGSTDNOW)) + LOGSTDNOW,axis=-1))

        # Defining Loss = - J is equivalent to max J
        pg_mean_losses = tf.maximum(-ratio_mean * ADV,-tf.clip_by_value(ratio_mean, 1.0 - CLIPRANGE, 1.0 + CLIPRANGE) * ADV) #* tf.minimum(1.0,RHO_NOW)

        pg_std_losses = tf.maximum(-ratio_std * ADV,-tf.clip_by_value(ratio_std, 1.0 - CLIPRANGE, 1.0 + CLIPRANGE) * ADV)  #* tf.minimum(1.0,RHO_NOW)
        ...
        clipfrac = tf.reduce_mean(tf.to_float(tf.greater(tf.abs(ratio_std - 1.0), CLIPRANGE)))
        pg_loss = tf.reduce_mean(pg_mean_losses+pg_std_losses)# * tf.minimum(1.0,RHO_NOW))
        if adaptive_kl:
        ...
        def train(lr, cliprange, klconst, rgae, trunc_rho, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, kl_rest, rho_now, states=None):
            ...
            td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                    CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values, MEANNOW:mean_now, LOGSTDNOW:logstd_now, KLCONST:klconst, KL_REST:kl_rest}
            if states is not None:
                td_map[train_model.S] = states
                td_map[train_model.M] = masks
            return sess.run(
                [pg_loss, vf_loss, entropy, approxkl, clipfrac, kloldnew, _grad_norm, _train],
                td_map
            )[:-1]
        self.loss_names = ['policy_loss', 'value_loss', 'policy_entropy', 'approxkl', 'clipfrac', 'kloldnew', 'gradnorm']
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, clipcut = None, trunc_rho=1.0, useadv = 0, vtrace = 0, rgae=0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    def add_vtarg_and_adv_vtrace4(seg, gamma, value, rho, trunc_rho, acdim=None):
        """
        Compute target value using TD(lambda) estimator, and advantage with GAE(lambda)
        """
        done = np.append(seg["done"],
        ...
        for t in reversed(range(T)):
            ...
            for j in range(t,T):
                if done[j+1]:
                    idx = j+1
                break
            gaelam[t] = np.sum(gamlam[:idx-t] *(np.minimum(1.0,c)*delta)[t:idx])
            c[t:] = rho_[t] * c[t:]
    ...
    for update in range(1, nupdates+1):
        ...
        if vtrace==1:
            ...
            adv, ret, gae = add_vtarg_and_adv_vtrace4(seg, gamma, values, rho, trunc_rho)
            if useadv:
                gae=adv
        else:
        ...
        for i in range(int(len(prior_row)/nsteps)):
            temp_row=np.mean(np.abs(rho[i * nsteps:(i + 1) * nsteps] - 1.0) + 1.0)
        ...
        for _ in range(noptepochs):
            ...
            for _ in range(int(nsteps/nbatch_train)):
                ...
                slices = (arr[idx] for arr in (runner.obfilt(seg["ob"]), ret, gae, seg["done"], seg["ac"], values[:-1], seg["neglogp"], seg["mean"], seg["logstd"], kl_rest, rho))
