...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        # vpredclipped = OLDVPRED + tf.clip_by_value(train_model.vf - OLDVPRED, - 0.2, 0.2)
        # Unclipped value
        ...
        # clipdim
        # ratio = tf.exp(- 0.5 * tf.square((A - mean) / tf.exp(logstd)) - logstd + 0.5 * tf.square((A - MEANNOW) / tf.exp(LOGSTDNOW)) + LOGSTDNOW)
        #
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, adaptive_clip=0, trunc_rho=1.0, useadv = 0, vtrace = 0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    policy = build_policy(env, network, squash=False,**network_kwargs)
    ...
    for update in range(1, nupdates+1):
        ...
        logrho = np.log(rho)
        state_prob = np.zeros(len(logrho))
        for i in range(len(logrho) - 1):
            if seg["done"][i]:
                state_prob[i + 1] = 0
            else:
                state_prob[i + 1] = state_prob[i] + np.minimum(0.0,logrho[i])
        state_prob = np.exp(state_prob)
        print("state prob")
        print(state_prob)
        ...
        for _ in range(noptepochs):
            ...
            for _ in range(int(2048/64)):
                if nbatch_adapt1 > 0:
                    idx1 = np.random.choice(inds1, nbatch_adapt1, p=state_prob[:-2048]/np.sum(state_prob[:-2048]))
                idx2 = np.random.choice(inds2, nbatch_adapt2, p=state_prob[-2048:]/np.sum(state_prob[-2048:]))
                # print(np.mean(np.abs(rho[mbinds] - 1.0) + 1.0))
                idx = np.hstack([idx1,idx2]).astype(int)

                slices = (arr[idx] for arr in (seg["ob"], ret, gae, seg["done"], seg["ac"], values[:-1], neglogpnow, mean_now, logstd_now, rho, kl_rest))
            ...
            # print(np.shape(seg["ac"])[1])
            # rho_after = np.exp(- 0.5 * np.square((seg["ac"] - mean_n) / np.exp(logstd_n)) - logstd_n + 0.5 * np.square((seg["ac"] - seg["mean"]) / np.exp(seg["logstd"])) + seg["logstd"])
            ...
            temp=[]
            for i in range(int(len(prior_row) / 2048)):
                temp_row = np.mean(np.abs(rho_after[i * 2048:(i + 1) * 2048] - 1.0) + 1.0, axis=0)
                # local_rho[i + (ERlen-int(len(prior_row)/2048))].append(temp_row)
                # print(temp_row)
