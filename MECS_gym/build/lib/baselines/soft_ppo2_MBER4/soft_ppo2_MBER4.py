...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        # Unclipped value
        vf_losses1 = tf.square(vpred - tf.stop_gradient(R))
        # Clipped value
        vf_losses2 = tf.square(vpredclipped - tf.stop_gradient(R))
        ...
        D_s = A.shape.as_list()[-1]
        policy_prior = tf.contrib.distributions.MultivariateNormalDiag(
            loc=tf.zeros(D_s), scale_diag=tf.ones(D_s))
        policy_prior_log_probs = policy_prior.log_prob(A)
        SOFT_ADV = tf.stop_gradient( ADV + 0.01*(neglogpac + policy_prior_log_probs) )
        # Defining Loss = - J is equivalent to max J
        pg_losses = - tf.reduce_prod(ratio, axis=-1) * SOFT_ADV # * tf.minimum(1.0,RHO_NOW)

        pg_losses2 = - tf.reduce_prod(tf.clip_by_value(ratio, 1.0 - CLIPRANGE, 1.0 + CLIPRANGE),axis=-1) * SOFT_ADV  # * tf.minimum(1.0,RHO_NOW)
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, adaptive_clip=0, trunc_rho=1.0, useadv = 0, vtrace = 0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    policy = build_policy(env, network, varstd=False, **network_kwargs)
    ...
    def add_vtarg_and_adv_vtrace_soft(seg, gamma, value, rho, trunc_rho, reward_scale, entropy, logp, priorlogp):
        """
        Compute target value using TD(lambda) estimator, and advantage with GAE(lambda)
        """
        done = np.append(seg["done"], 0)  # last element is only used for last vtarg, but we already zeroed it if last new = 1
        ...
        print(rew)
        print(logp - priorlogp)
        for t in reversed(range(T)):
            nonterminal = 1 - done[t + 1]
            delta = r[t] * (rew[t] - 0.01*(logp - priorlogp)[t] + gamma * value[t + 1] * nonterminal - value[t])
            gaelam[t] = lastgaelam = delta + gamma * c[t] * nonterminal * lastgaelam
        ret = gaelam + value[:-1]
        adv = rew + gamma * (1.0-done[1:]) * np.hstack([ret[1:], value[T]]) - value[:-1]
        return adv, ret, gaelam
    ...
    for update in range(1, nupdates+1):
        ...
        zero = np.zeros(np.shape(seg["ac"]))
        priorneglogp = 0.5 * np.sum(np.square((seg["ac"] - zero) / np.exp(zero)), axis=-1) \
                      + 0.5 * np.log(2.0 * np.pi) * np.shape(seg["ac"])[1] \
                      + np.sum(zero, axis=-1)
        print(priorneglogp)
        rho = np.exp(-neglogpnow + seg["neglogp"])
        _,logstd_ent = model.meanlogstds(ob_stack)
        entropy = np.sum(logstd_ent + .5 * np.log(2.0 * np.pi * np.e), axis=-1)
        # print(len(mean_now))
        # print(cliprangenow)
        # print(rho)
        if vtrace:
            adv, ret, gae = add_vtarg_and_adv_vtrace_soft(seg, gamma, values, rho, trunc_rho, 5.0, entropy, -neglogpnow, -priorneglogp)
            # print(adv)
