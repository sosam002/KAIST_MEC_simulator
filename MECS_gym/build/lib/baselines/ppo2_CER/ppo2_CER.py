...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        approxkl = .5 * tf.reduce_mean(tf.square(neglogpac - OLDNEGLOGPAC)* tf.minimum(1.0, RHO_NOW))
        ...
        def train(lr, cliprange, klconst, rgae, trunc_rho, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, kl_rest, rho_now, states=None):
            ...
            td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                    CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values, MEANNOW:mean_now, LOGSTDNOW:logstd_now, KLCONST:klconst, KL_REST:kl_rest, RHO_NOW:rho_now}
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, trunc_rho=1.0, useadv = 0, vtrace = 0, rgae=0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    for update in range(1, nupdates+1):
        ...
        for _ in range(noptepochs):
            ...
            for _ in range(int(nsteps/nbatch_train)):
                ...
                slices = (arr[idx] for arr in (runner.obfilt(seg["ob"]), ret, gae, seg["done"], seg["ac"], values[:-1], neglogpnow, seg["mean"], seg["logstd"], kl_rest, rho))
