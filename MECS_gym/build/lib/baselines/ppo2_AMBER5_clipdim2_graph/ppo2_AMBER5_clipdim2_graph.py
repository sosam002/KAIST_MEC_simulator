...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        def train(lr, cliprange, klconst, rgae, trunc_rho, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, kl_rest, rho_now, rho_dim, neglogpnow, states=None):
            ...
            notuse_num = np.sum(((advs > 0) & np.prod(rho_dim > 1 + 0.2,axis=1)) + ((advs < 0) & np.prod(rho_dim < 1 - 0.2, axis=1)))
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, trunc_rho=1.0, clipcut=0.2, useadv = 0, vtrace = 0, rgae=0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        batch_IS_dim_epoch = np.zeros(noptepochs)
        ...
        for epo in range(noptepochs):
            ...
            rho_ = np.exp(-neglogpn + seg["neglogp"])
            batch_rho = np.abs(1.0 - rho_) + 1.0
            rho_mean = np.mean(batch_rho)
            batch_IS_epoch[epo] = rho_mean

            rho_dim = np.exp(- 0.5 * np.square((seg["ac"] - mean_n) / np.exp(logstd_n)) \
                          - logstd_n + 0.5 * np.square((seg["ac"] - seg["mean"]) / np.exp(seg["logstd"]))\
                          + seg["logstd"])
            batch_rho_dim = np.abs(1.0 - rho_dim) + 1.0
            rho_dim_mean = np.mean(batch_rho_dim)
            batch_IS_dim_epoch[epo] = rho_dim_mean
            ...
            for _ in range(int(nsteps/nbatch_train)):
                if nbatch_adapt1>0:
                    idx1 = np.random.choice(inds1, nbatch_adapt1,p=prior_row[:-2048]/np.sum(prior_row[:-2048]))
                idx2 = np.random.choice(inds2, nbatch_adapt2)
                # print(np.mean(np.abs(rho[mbinds] - 1.0) + 1.0))
                idx = np.hstack([idx1,idx2]).astype(int)
                slices = (arr[idx] for arr in (ob, ret, gae, seg["done"], seg["ac"], values[:-1], seg["neglogp"], seg["mean"], seg["logstd"], kl_rest, rho, rho_dim, neglogpnow))
                loss_epoch, notuse_num = model.train(lrnow, cliprangenow, klconst, rgae, trunc_rho, *slices)
                mblossvals.append(loss_epoch)
                losses_epoch.append(loss_epoch)
                notuse_frac += notuse_num
            notuse_frac /= ((nbatch_adapt1 + nbatch_adapt2) * (nsteps / nbatch_train))
        ...
        batch_IS_dim.append(batch_IS_dim_epoch)
        ...
        if save_interval and (update % save_interval == 0 or update == 1) and logger.get_dir() and (MPI is None or MPI.COMM_WORLD.Get_rank() == 0):
            ...
            model.save(savepath)
    np.savetxt('/home/han/Downloads/log_ppo/graphdata/batchIS_DISC_%s_Humanoid.txt'%str(dtarg), np.array(batch_IS))
    np.savetxt('/home/han/Downloads/log_ppo/graphdata/batchISdim_DISC_%s_Humanoid.txt'%str(dtarg), np.array(batch_IS_dim))
    np.savetxt('/home/han/Downloads/log_ppo/graphdata/notuse_DISC_%s_Humanoid.txt'%str(dtarg), np.array(not_use))
    return model
