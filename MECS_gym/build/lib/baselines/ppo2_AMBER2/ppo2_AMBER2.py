...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        pg_loss = tf.reduce_mean(tf.maximum(pg_losses, pg_losses2)+KLCONST *approxkl)
        # Total loss
        ...
        def train(lr, cliprange, klconst, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, states=None):
            # Here we calculate advantage A(s,a) = R + yV(s') - V(s)
            # Returns = R + yV(s')
            # Normalize the advantages
            advs = (advs - advs.mean()) / (advs.std() + 1e-8)
            td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                    CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values, MEANNOW:mean_now, LOGSTDNOW:logstd_now, KLCONST:klconst}
            if states is not None:
                td_map[train_model.S] = states
                td_map[train_model.M] = masks
            return sess.run(
                [pg_loss, vf_loss, entropy, approxkl, clipfrac, kloldnew, _train],
                td_map
            )[:-1]
        self.loss_names = ['policy_loss', 'value_loss', 'policy_entropy', 'approxkl', 'clipfrac', 'kloldnew']
...
def learn(*, network, env, total_timesteps, dtarg=0.005, adaptive_kl=0, adaptive_clip=0, trunc_rho=1.0, vtrace = 0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        rho = np.exp(- 0.5 * np.sum(np.square((seg["ac"] - mean_now) / np.exp(logstd_now)), axis=-1) \
                      - 0.5 * np.log(2.0 * np.pi) * np.shape(seg["ac"])[1] \
                      - np.sum(logstd_now, axis=-1) + seg["neglogp"] )
        # print(len(mean_now))
        # print(cliprangenow)
        # print(rho)
        if vtrace:
            adv, ret = add_vtarg_and_adv_vtrace(seg, gamma, values, rho, trunc_rho)
        else:
            adv, ret = add_vtarg_and_adv(seg, gamma, values, lam)
        ...
        inds = np.arange(len(seg["ob"]))
        print(len(seg["ob"]))
        print(cliprangenow)
        nbatch_adapt = int(np.sum(prior_row) / nsteps * nbatch_train)
        for _ in range(noptepochs):
            ...
            for _ in range(int(2048/64)):
                idx = np.random.choice(inds, nbatch_adapt, p=prob)
                # print(np.mean(np.abs(rho[mbinds] - 1.0) + 1.0))
                slices = (arr[idx] for arr in (seg["ob"], ret, adv, seg["done"], seg["ac"], values[:-1], seg["neglogp"], seg["mean"], seg["logstd"]))
                mblossvals.append(model.train(lrnow, cliprangenow, klconst, *slices))
