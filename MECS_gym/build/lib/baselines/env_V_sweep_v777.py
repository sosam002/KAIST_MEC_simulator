...
class MEC_v7MB(gym.Env):
    ...
    def reset(self, empty_reward=True, rand_start = 0):
        ...
        _, failed_to_generate, _ = self._step_generation(scale=MB)
        reset_state = self._get_obs(scale=MB)
        return reset_state[:-4]
    ...
    def step(self, action, use_beta=True, generate=True, scale=MB):
        # print(action)
        action = np.clip(action,-1.0,1.0)
        action = 5 * action
        start_state = self._get_obs(scale=scale)
        q1=np.array(self.get_edge_qlength(scale=scale))
        ...
        q_increase = np.array(q_last) - q1
        _, failed_to_generate, _ = self._step_generation()

        new_state= self._get_obs(scale=scale)
        cost = self.get_cost(used_edge_cpus, used_cloud_cpus, q1, q_last, cost_type=self.cost_type)
        self.timestamp += 1
        if self.timestamp == self.max_episode_steps:
            return new_state[:-4], -cost, 1, {"cloud_cpu_used":start_state[-1]}
        return new_state[:-4], -cost, 0, {"cloud_cpu_used":start_state[-1]}

    def _step_alpha(self, action, scale=MB):
        ...
        state = self._get_obs(scale=scale)
    ...
    def _step_beta(self, action, scale=MB):
        ...
        q_before = self.get_edge_qlength(scale=scale)
        # print(q_before)
        ...
        for server_id, server in self.servers.items():
            ...
            s2 = self._get_obs(scale=scale)
            used_cloud_cpus[server_id] = server.do_tasks()
            used_cloud_cpus[server_id] = (s2-s1)[-2]*scale
        # print(s1)
        # print(s2)
        # print(GHZ)
    ...
    def _step_generation(self, scale=MB):
        initial_qlength= self.get_edge_qlength(scale=scale)
        ...
        after_qlength = self.get_edge_qlength(scale=scale)

        return initial_qlength, failed_to_generate, after_qlength

    def get_edge_qlength(self, scale=MB):
        qlengths = list()
        # if self.timestamp % 1000 == 0:
    ...
    def get_cost(self, used_edge_cpus, used_cloud_cpus, before, after, cost_type, failed_to_offload=0, failed_to_generate=0):
        def compute_cost_fct(cores, cpu_usage):
            return cores*(cpu_usage/40/GHZ/cores)**3

        # every queue lengths are in unit of  [10e9 bits]
        q_len_sum_after = sum(np.array(after)**2)
        q_len_sum_before = sum(np.array(before)**2)
