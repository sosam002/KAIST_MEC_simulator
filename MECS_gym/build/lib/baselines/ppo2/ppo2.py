...
def learn(*, network, env, total_timesteps, sess=None, fixstd=False, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        # Get minibatch
        if fixstd:
            frac_ = np.maximum(1.0-(update-1.0)/1000,0)
            sess.run(logstd_var.assign((frac_-1)*np.ones(shape=(1,acdim))))
