...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        RCONST = tf.placeholder(tf.float32, [])
        ...
        pg_losses2 = -tf.clip_by_value(ratio, 2.0 - ((1.0 + CLIPRANGE) * RCONST), (1.0 + CLIPRANGE) * RCONST) * ADV #* tf.minimum(1.0,RHO_NOW)
        ...
        pg_loss = tf.reduce_mean(tf.maximum(pg_losses, pg_losses2)/RCONST + KLCONST*approxkl)# * tf.minimum(1.0,RHO_NOW))
        # Total loss
        ...
        def train(lr, cliprange, klconst, rconst, obs, returns, advs, masks, actions, values, neglogpacs, mean_now, logstd_now, rho_now, kl_rest, states=None):
            # Here we calculate advantage A(s,a) = R + yV(s') - V(s)
            # Returns = R + yV(s')
            # Normalize the advantages
            advs = (advs - advs.mean()) / (advs.std() + 1e-8)
            td_map = {train_model.X:obs, A:actions, ADV:advs, R:returns, LR:lr,
                    CLIPRANGE:cliprange, OLDNEGLOGPAC:neglogpacs, OLDVPRED:values, MEANNOW:mean_now, LOGSTDNOW:logstd_now, KLCONST:klconst, RHO_NOW:rho_now, KL_REST:kl_rest, RCONST:rconst}
...
class EvalRunner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            # Given observations, get action value and neglopacs
            # We already have self.obs because Runner superclass run self.obs[:] = env.reset() on init
            actions, logstds = self.model.meanlogstd(self.obs)
            mb_obs.append(self.obs.copy())
            mb_actions.append(actions)
            mb_dones.append(self.dones)
...
def learn(*, network, env, total_timesteps, dtarg=0.01, adaptive_kl=0, adaptive_clip=0, trunc_rho=1.0, useadv = 0, vtrace = 0, eval_env = None, seed=None, ERlen=1, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        kl_rest[:-nsteps]=0
        rconst = np.mean(np.abs(rho - 1.0) + 1.0)
        # print(kl_rest)
        for _ in range(noptepochs):
            ...
            for _ in range(int(nsteps/nbatch_train)):
                ...
                loss_epoch = model.train(lrnow, cliprangenow, klconst, rconst, *slices)
            ...
            rho_after = np.exp(-neglogpn + seg["neglogp"])
            prior_row = np.zeros(len(seg["ob"]))
            temp=[]
            for i in range(int(len(prior_row) / nsteps)):
            ...
            print(rconst)
            print(temp)
            rconst = np.mean(np.abs(rho_after - 1.0) + 1.0)
