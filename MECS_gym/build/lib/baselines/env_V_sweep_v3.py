...
class MEC_v3(gym.Env):
    ...
    def reset(self, empty_reward=True, rand_start = 0):
        ...
        reset_state = np.array(list(self._get_obs(scale=GHZ))+[self.timestamp%5000])
        return reset_state
    ...
    def step(self, action, use_beta=True, generate=True):
        ...
        new_state= np.array(list(self._get_obs(scale=GHZ)) + [self.timestamp%5000])
    ...
    def get_cost(self, used_edge_cpus, used_cloud_cpus, before, after, cost_type, failed_to_offload=0, failed_to_generate=0):
        ...
        edge_drift_cost = (q_len_sum_after-q_len_sum_before) * (5000-(self.timestamp%5000))
        # if self.timestamp%1000==0:
        #     print("q len : ", after)
        ...
        # edge_computation_cost=0
        # cloud_payment_cost =0
        return (edge_drift_cost+cost_type*(edge_computation_cost+cloud_payment_cost))/100